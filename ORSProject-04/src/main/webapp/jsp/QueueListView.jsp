<%@page import="in.co.rays.proj4.controller.QueueListCtl"%>
...
<body>
	...
	<div align="center">
		<h1 align="center" style="margin-bottom: -15; color: navy;">Queue
			List</h1>
		...
		<form action="<%=ORSView.QUEUE_LIST_CTL%>" method="post">
			...
			<%
				...
				List<QueueBean> list =
				(List<QueueBean>) ServletUtility.getList(request);

				if (list != null && !list.isEmpty()) {

					Iterator<QueueBean> it = list.iterator();
			%>
			...
			<table style="width: 100%">
				<tr>
					<td align="center">
						...
						<label><b>Queue Code :</b></label>
						<input type="text" name="queueCode"
							placeholder="Enter Queue Code"
							value="<%=ServletUtility.getParameter("queueCode", request)%>">&emsp;

						<label><b>Queue Name :</b></label>
						<input type="text" name="queueName"
							placeholder="Enter Queue Name"
							value="<%=ServletUtility.getParameter("queueName", request)%>">&emsp;

						<label><b>Status :</b></label>
						<%=HTMLUtility.getList("queueStatus",
								String.valueOf(bean.getQueueStatus()),
								statusMap)%>&emsp;
			...
			<table border="1" style="width: 100%; border: groove;">
				<tr style="background-color: #e1e6f1e3;">
					...
					<th width="5%">S.No</th>
					<th width="15%">Queue Code</th>
					<th width="20%">Queue Name</th>
					<th width="10%">Queue Size</th>
					<th width="10%">Queue Status</th>
					<th width="5%">Edit</th>
				</tr>

				<%
					while (it.hasNext()) {
						bean = (QueueBean) it.next();
				%>
				...
				<tr>
					...
					<td><%=index++%></td>
					<td><%=bean.getQueueCode()%></td>
					<td><%=bean.getQueueName()%></td>
					<td><%=bean.getQueueSize()%></td>
					<td><%=bean.getQueueStatus()%></td>

					<td style="text-align: center;">
						<a href="QueueCtl?id=<%=bean.getId()%>">Edit</a>
					</td>
				</tr>
